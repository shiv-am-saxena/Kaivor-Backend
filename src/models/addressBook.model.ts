...
import IAddressBook from '../types/schema/user.js';

const addressBookSchema = new Schema<IAddressBook>({
    name: { type: String, required: true, message: 'Name is required' },
    phoneNumber: { type: String, required: true, match: [/^\+\d{1,15}$/, 'Phone number is required with country code'] },
    address: {
        type: {
            type: String,
...
const AddressBookModel = model<IAddressBook>('AddressBook', addressBookSchema);
